...
class FuzzImageObserver final
 ...
 public:
  ...
  void Trace(Visitor* visitor) const override { ImageObserver::Trace(visitor); }
};
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
  ...
  // IMPORTANT: new image per input (SetData() can't be reused safely)
  scoped_refptr<SVGImage> image = SVGImage::Create(g_observer);

  scoped_refptr<SharedBuffer> buffer =
      SharedBuffer::Create(base::span<const uint8_t>(data, size));

  image->SetData(buffer, true);
  (void)image; // "Use" the reference...
  // Do not draw...
